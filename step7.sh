#!/usr/bin/env bash
#
# step7.sh: Hour 7 - 통합 테스트 & 디버깅
# - Creates test_end_to_end.py, which runs:
#   1) .env load
#   2) update_documents_if_needed (force re-update)
#   3) RAG query test
#

echo "=== Start: Hour 7 - Creating end-to-end test script. ==="

cat <<'PYTHON' > project/test_end_to_end.py
#!/usr/bin/env python3
...
chmod +x project/test_end_to_end.py

echo "=== Done. Created test_end_to_end.py. ==="
echo "Usage:"
echo "  cd project"
echo "  source ../.venv/bin/activate"
echo "  python test_end_to_end.py"
echo "If everything goes well, you'll see GPT_ANSWER_STUB in the final RAG result."
